...
export class ContractsService {
  ...
  async findAll(subAccount: string, query: ContractListQueryDto, baseUrl: string): Promise<ContractListResponseDto> {
    ...
    const whereClause: any = {
      ...
      ...(query.search && {
        OR: [
          { contract_type: { contains: query.search, mode: 'insensitive' as const } },
          { contract_number: { contains: query.search, mode: 'insensitive' as const } },
          { contract_beneficiary_name: { contains: query.search, mode: 'insensitive' as const } },
          { contract_merchant_id: { contains: query.search, mode: 'insensitive' as const } },
          { contract_alias_id: { contains: query.search, mode: 'insensitive' as const } },
        ],
      }),
      ...(query.contract_type && { contract_type: query.contract_type }),
      ...(query.created_before && { contract_created_at: { lt: new Date(query.created_before) } }),
      ...(query.created_after && { contract_created_at: { gt: new Date(query.created_after) } }),
      ...(query.last_modified_before && { contract_last_modified: { lt: new Date(query.last_modified_before) } }),
      ...(query.last_modified_after && { contract_last_modified: { gt: new Date(query.last_modified_after) } }),
    };
    ...
    const pagination: PaginationResponseDto = {
      ...
      total_pages,
      total_count,
      next_link: page < total_pages ? `${baseUrl}/contracts?page=${page + 1}&per_page=${per_page}` : null,
      prev_link: page > 1 ? `${baseUrl}/contracts?page=${page - 1}&per_page=${per_page}` : null,
    };
  ...
  async findByTerminal(terminalId: string, subAccount: string, query: ContractListQueryDto, baseUrl: string): Promise<ContractListResponseDto> {
    const { page = 1, per_page = 10 } = query;
    const skip = (page - 1) * per_page;
    const take = per_page;
    ...
    const pagination: PaginationResponseDto = {
      ...
      total_pages,
      total_count,
      next_link: page < total_pages ? `${baseUrl}/contracts/terminal/${terminalId}?page=${page + 1}&per_page=${per_page}` : null,
      prev_link: page > 1 ? `${baseUrl}/contracts/terminal/${terminalId}?page=${page - 1}&per_page=${per_page}` : null,
    };
  ...
  async findByAccount(accountId: string, subAccount: string, query: ContractListQueryDto, baseUrl: string): Promise<ContractListResponseDto> {
    const { page = 1, per_page = 10 } = query;
    const skip = (page - 1) * per_page;
    const take = per_page;
    ...
    const pagination: PaginationResponseDto = {
      ...
      total_pages,
      total_count,
      next_link: page < total_pages ? `${baseUrl}/contracts/account/${accountId}?page=${page + 1}&per_page=${per_page}` : null,
      prev_link: page > 1 ? `${baseUrl}/contracts/account/${accountId}?page=${page - 1}&per_page=${per_page}` : null,
    };
